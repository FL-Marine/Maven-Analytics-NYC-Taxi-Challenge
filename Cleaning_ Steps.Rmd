...
```{r}
select(taxi_data_join_2, -lpep_pickup_datetime, -lpep_dropoff_datetime, -hour, -minute, -second, -date)
```
...
taxi_data_join_3 <- taxi_data_join_2 %>% 
  ...
  relocate(service_date, .before = PULocationID) %>% 
  relocate(PU_time, .before = PULocationID) %>% 
  relocate(DO_time, .before = DOLocationID)
```

This didn't work for some reason to remove columns
```{r}
taxi_data_join_4 <- taxi_data_join_3  
select(taxi_data_join_4, -lpep_pickup_datetime, -lpep_dropoff_datetime, -hour, -minute, -second, -date)
```

Removed columns using the below method again.
```{r}
taxi_data_join_4$lpep_pickup_datetime <- NULL
taxi_data_join_4$lpep_dropoff_datetime <- NULL
taxi_data_join_4$hour <- NULL
taxi_data_join_4$minute <- NULL
taxi_data_join_4$second <- NULL
taxi_data_join_4$date <- NULL
```
...
```{r}
fwrite(taxi_data_join_4, "C:/Users/abedi/OneDrive/Documents/R Projects/Maven Analytics NYC Taxi Challenge/clean_data_2.csv")
```
